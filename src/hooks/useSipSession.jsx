...
const useSipSession = (audioRef) => {
  ...
  const makeCall = ({ phone, selectedNumber, onSession }) => {
    ...
    if (prevActiveLineId !== null && 
        linesRef.current[prevActiveLineId]?.session?.state === SessionState.Established &&
      ...
      try {
        ...
        setHoldState(linesRef.current[prevActiveLineId].session, true)
          .then(() => console.log(`✅ Put previous active line ${prevActiveLineId} on hold before dialing new call`))
          .catch(error => console.warn(`❗ Failed hold operation on line ${prevActiveLineId}:`, error));
        
      } catch (err) {
        console.warn(`❗ Failed to hold line ${prevActiveLineId} before switching`, err);
    ...
    // Ensure mute state is reset for the new call
