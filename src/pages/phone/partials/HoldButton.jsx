...
const HoldButton = ({ sessionRef }) => {
  const isOnHoldRedux = useSelector((state) => state.callFeature.isOnHold) || false;
  ...
  useEffect(() => {
    if (canHold && session) {
      const checkHoldState = () => {
        ...
        const isOnHold = senders.some(sender => 
          sender.track?.kind === "audio" && 
          pc.getTransceivers().find(t => t.sender === sender)?.direction === "sendonly"
        );
