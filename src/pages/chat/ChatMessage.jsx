...
export default function WhatsAppStyleChat() {
  ...
  const fetchChatMessages = async (roomId) => {
    ...
    try {
      ...
      const response = await getChatMessages(roomId, { signal: controller.signal });
      ...
      if (error?.name === "CanceledError" || error?.code === "ERR_CANCELED" || error?.message === "canceled" || error?.name === "AbortError") {
        // Axios abort/cancel, do nothing
      } else {
        console.error("Error fetching chat messages:", error);
        errorToast(error?.response?.data?.message || "Failed to fetch messages");
        setMessages([]);
      }
    } finally {
      setTimeout(() => {
  ...
  useEffect(() => {
    if (messages.length > 0 && !messagesLoading && messagesContainerRef.current) {
      messagesContainerRef.current.scrollTop = messagesContainerRef.current.scrollHeight;
    }
  ...
  useEffect(() => {
    getChatListhandler();
    if (newSocketData?.room === roomId) {
      fetchChatMessages(newSocketData.room);
    }
  }, [newSocketData]);
  
  // Modify the handleContactSelection function to better handle loading state
  const handleContactSelection = (room, displayName) => {
    if (!room?.room_name) return;
    
    // Set loading state when switching users
    setMessagesLoading(true);
    setMessages([]); // Clear previous messages immediately for better UX
    ...
    if (myNumber && otherNumber) {
      ...
      setMessagesLoading(false); // Reset loading state on error
    }
  };
  ...
  const filteredContacts = contacts?.chat_rooms
    ? contacts.chat_rooms.filter((room) => {
  ...
  return (
    <section
      className="w-[calc(100%-32px)] m-4 bg-white grid grid-cols-4 rounded-2xl"
      style={{ boxShadow: "0px 0px 15px rgba(0, 0, 0, 0.1)" }}
    >
      {/* Sidebar */}
      <div className="col-span-1 bg-white p-4 border-[1px] border-gray-200 overflow-y-auto rounded-l-2xl">
        <div className="flex items-center mb-4 justify-between">
          <input
            type="text"
            placeholder="Search contacts..."
            className="flex-1 border-[1px] px-3 py-2 rounded-[6px] focus:outline-none border-gray-200"
            value={search}
        ...
        <div className="h-[calc(100vh-170px)] overflow-y-auto overflowScroll">
          {loading && firstLoad ? (
      ...
      {/* Chat Window */}
      <div className="col-span-3 flex flex-col h-[calc(100vh-80px)]">
        {openNewChat ? (
          <>
            <div
              className="sticky top-0 z-10 bg-white border-b-[1px] border-gray-200 p-4 py-5 flex items-center justify-between gap-3 rounded-r-2xl"
              style={{ boxShadow: "0px 0px 15px rgba(0, 0, 0, 0.1)" }}
            >
              <div className="relative flex items-center justify-start gap-2.5 w-full">
                ...
                <button
                  className="text-sm bg-secondary !text-white px-4 py-2.5 rounded flex items-center gap-2 cursor-pointer"
            ...
            >
              ...
              <div className="flex gap-2 items-center">
                ...
                <label htmlFor="file-upload">
                  <img
                    src={attachFile}
                    className="max-w-5 cursor-progress"
                    alt=""
                  />
                </label>
                <button
          ...
          <>
            <div
              className="sticky top-0 z-10 bg-white border-b-[1px] border-gray-200 p-4 py-5 flex items-center justify-between gap-3"
              style={{ boxShadow: "0px 0px 15px rgba(0, 0, 0, 0.1)" }}
            >
              ...
              <div className="flex flex-col justify-end">
                {messagesLoading ? (
                  ...
                  dateKeys.map((dateKey) => {
                    ...
                    return (
                      <div key={dateKey}>
                        <div className="flex justify-center my-2 relative border-b-[1px] border-gray-200">
                          ...
                          return (
                            ...
                            >
                              <div className="max-w-[80%]">
                                ...
                                {isSender && (
                                  <span
                                    className={`text-xs justify-end flex mb-2.5 ${
                                      msg.read
                                        ? "text-gray-500"
                                        : "text-primary"
                                    }`}
