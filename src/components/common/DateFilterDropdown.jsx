...
const DateFilterDropdown = ({ selectedFilter, onFilterChange, onCustomDateChange, isLoading = false }) => {
    ...
    if (customDates.startDate && customDates.endDate) {
      ...
      const formattedDates = {
        ...
        endDate: isEndDateToday 
          ? `${customDates.endDate} ${String(today.getHours()).padStart(2, '0')}:${String(today.getMinutes()).padStart(2, '0')}` 
  ...
  const getSelectedLabel = () => {
    if (selectedFilter === "custom" && customDates.startDate && customDates.endDate) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {showCustomDate && (
        <div className="absolute z-10 w-72 p-4 mt-1 bg-white border border-gray-200 rounded-md shadow-lg">
          ...
          <form onSubmit={handleCustomDateSubmit}>
            ...
            <div className="flex justify-end space-x-2">
              ...
              <button
                type="submit"
                className={`px-3 py-1 text-xs bg-secondary text-white rounded ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                disabled={!customDates.startDate || !customDates.endDate || isLoading}
              >
                Apply
              </button>
            </div>
